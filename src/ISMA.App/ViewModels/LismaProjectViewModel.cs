...
public partial class LismaProjectViewModel : ObservableObject, IProjectViewModel
{
    ...
    {
        if (_editorInstance != null && !ReferenceEquals(_editorInstance, editor))
        {
    ...
    {
        var editor = (_editorInstance as IsmaTextEditor)?.Editor;
        if (editor is null || editor.SelectionLength <= 0)
        {
            CutRequested?.Invoke();
            return;
        }
    ...
    {
        (_editorInstance as IsmaTextEditor)?.Editor.SelectAll();
    }
